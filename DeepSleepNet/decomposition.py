...
for i in range(len(N1_list)):
    with np.load(join(N1_path,N1_list[i])) as npz:
        ...
        Delta = butter_lowpass_filter(raw,3.9,100)
        Theta = butter_bandpass_filter(raw,3.9,7.8,100)
        Alpa  = butter_bandpass_filter(raw,7.8,15.6,100)
        Beta  = butter_bandpass_filter(raw,15.6,31.3,100)
        data = np.concatenate((raw,Delta,Theta,Alpa,Beta),axis=0)
